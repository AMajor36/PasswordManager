...
public class MasterPasswordServices
{
    ...
    public void ChangeMasterPassword(string oldMasterPassword, string newMasterPassword)
    {
        ...
        if (!Cryptography.VerifyPassword(oldMasterPassword, 
        metadata.Salt, 
        metadata.PasswordVerificationHash))
        {
        ...
        {
            string serviceName = Encoding.UTF8.GetString(Cryptography.Decrypt(entry.EncryptedServiceName, 
            oldKey, 
            entry.ServiceNameIv));

            string username = Encoding.UTF8.GetString(Cryptography.Decrypt(entry.EncryptedUsername, 
            oldKey, 
            entry.UsernameIv));

            string password = Encoding.UTF8.GetString(Cryptography.Decrypt(entry.EncryptedPassword, 
            oldKey, 
            entry.PasswordIv));

            entry.EncryptedServiceName = Cryptography.Encrypt(Encoding.UTF8.GetBytes(serviceName), 
            newKey, 
            out var newServiceNameIv);
            entry.ServiceNameIv = newServiceNameIv;

            entry.EncryptedUsername = Cryptography.Encrypt(Encoding.UTF8.GetBytes(username), 
            newKey, 
            out var newUsernameIv);
            entry.UsernameIv = newUsernameIv;
            
            entry.EncryptedPassword = Cryptography.Encrypt(Encoding.UTF8.GetBytes(password), 
            newKey, 
            out var newPasswordIv);
            entry.PasswordIv = newPasswordIv;
        }

        metadata.Salt = newSalt;
        metadata.PasswordVerificationHash = Cryptography.DeriveKey(newMasterPassword, 
        newSalt);
